...
void render(unsigned char *text, int nchr, unsigned char *buffer, int nbfr) {
...
int main(int argc, char **argv) {
  ...
  int nbfr = strlen(text) * 6;

  unsigned char *buffer = (unsigned char *)malloc(sizeof(unsigned char) * nbfr);
  render((unsigned char *)text, strlen(text), buffer, nbfr);

  for (int i = 0; i < 7; i++) {
    for (int j = 0; j < nbfr; j++) {
      if ((buffer[j] >> i) & 1) {
        printf("*");
      } else {
        printf(" ");
